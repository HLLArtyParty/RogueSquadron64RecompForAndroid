...
a keyboard alone clears the NO-CONTROLLER gate. Default map — **Enter = Start, Space = A
(fire/confirm)**, LShift = B, LCtrl = Z, E = Rtrig, Q = Ltrig, WASD/arrows = stick, IJKL = C,
numpad 8/4/2/6 = D-pad, Esc.
...
- `tools/drive-input.ps1 -Keys "enter:100:1500,space:100:800"` — send a key sequence to the running
...
From a fresh boot, this is the button path. Space = A (confirm), Enter = Start.

1. **Boot cinematic** (attribution -> N64 logo -> Factor 5). Tap **Enter** a few times, spaced
   ~1.6 s, to skip to the title / **SELECT GAME**.
2. **SELECT GAME** (3 save slots): **Space** on an empty slot -> ENTER NAME.
3. **ENTER NAME** (letter grid): **Space** picks the highlighted letter (tap a couple), then
   **Enter** finishes the name -> ARE YOU SURE.
4. **ARE YOU SURE? YES/NO** (YES highlighted): **Space** -> SELECT LEVEL.
5. **SELECT LEVEL** (holo table, e.g. "Ambush at Mos Eisley"): **Space** -> AVAILABLE CRAFT.
6. **AVAILABLE CRAFT** (holo ships for the level): **Space** selects the craft, then **one more
   Space** confirms the mission -> the **hangar** loads.
...
- Send input only after the window exists and is focused; a missed early tap just lands on the
  cinematic (harmless) — the extra spaced Enters absorb boot-time variance.
