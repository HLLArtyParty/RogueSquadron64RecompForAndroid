...
| `ROGUESQ_F5_TERRAIN_SEAMS` | on | Grid-tile LOD seams. Record bytes +4..+7 are the left/right/top/bottom neighbour LODs; when one differs from the tile's shift, the odd samples on that edge become the mean of their neighbours (no T-junction against a coarser neighbour), then blend toward the corner line by the edge weight (+0x16 L, +0x18 R, +0x1A T, +0x1C B). `0` disables. Replaces the old RDRAM level-grid stitch and all-rows geomorph. |
