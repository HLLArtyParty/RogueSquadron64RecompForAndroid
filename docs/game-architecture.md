...
1. **Stale material calls** (12 per run). The game's list calls a material chunk that has already been recycled, so the parser walks stale bytes. Serializing the parse against the game (`ROGUESQ_GFX_SYNC_SUBMIT=1`) does not change the count, so it is the game recycling earlier than on hardware, most likely tied to the recomp spending three VIs per frame instead of two (expiry workers tick per VI). The walk budgets end the task instead of stalling; the affected frame loses the rest of its list. Root cause open.
...
3. **Frame pacing.** Three VIs per frame vs two on hardware; parse time averages 10–15 ms with occasional 100–600 ms spikes (RT64 pipeline creation on garbage state from item 1 is the suspect).
